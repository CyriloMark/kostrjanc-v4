...
export default function EventCreate({ navigation, route }) {
  ...
  const openImagePickerAsync = async () => {
    ...
    let aspect = pickerResult.assets[0].width / pickerResult.assets[0].width;
  ...
  const setUnfullfilledAlert = () => {
    ...
    if (event.title.length === 0) missing += `\n${getLangs("missing_title")}`;
  ...
  const publishEvent = async () => {
    ...
    if (
      !fromLinking &&
      !(!checkForLinkings(event.title) && !checkForLinkings(event.description))
    ) {
    ...
    if (event.eventOptions.adBanner.uri !== undefined) {
    ...
    if (response.status == "accepted") {
      ...
      if (
        response.reason == "title or description includes/include bad words!"
      ) {
        Alert.alert(
  ...
  return (
    <View style={[style.container, style.bgBlack]}>
      ...
      >
        {/* Header */}
        <Pressable style={{ zIndex: 10 }} onPress={openDatePickerAndroid}>
          <BackHeader
        ...
        >
          ...
          <View>
            ...
            {/* Map */}
            <View style={[styles.mapContainer, style.allCenter, style.oHidden]}>
              <MapView
                ref={mapRef}
                style={style.allMax}
              ...
              <SVG_Pin fill={style.colors.red} style={styles.mapPin} />
            </View>
          ...
          <View style={styles.sectionContainer}>
            ...
            {/* Time Start-End */}
            <View style={[styles.underSectionContainer, styles.rowContainer]}>
            ...
            {/* Type */}
            {event.eventOptions.type !== undefined && checkedCategories.type ? (
              <View style={[styles.underSectionContainer, styles.rowContainer]}>
                <Text style={[style.Tmd, style.tWhite]}>
                  {getLangs("event_about_type")}{" "}
                  {getLangs(Event_Types[event.eventOptions.type])}
                </Text>
              </View>
            ) : null}
            ...
            {/* Website */}
            {event.eventOptions.website && checkedCategories.website ? (
              <Pressable
                style={styles.underSectionContainer}
                onPress={() => openLink(event.eventOptions.website)}
              >
                <Text style={[style.Tmd, style.tWhite]}>
                  {getLangs("event_about_website")} {event.eventOptions.website}
                </Text>
              </Pressable>
            ) : null}

            {/* Ad Banner */}
            {event.eventOptions.adBanner && checkedCategories.adBanner ? (
              <Pressable
                onPress={() =>
                ...
                style={[styles.underSectionContainer, styles.rowContainer]}
              >
                <Image
                  style={[
                    styles.adBanner,
          ...
          <View style={styles.sectionContainer}>
            ...
            <View style={[style.pH, { marginTop: style.defaultMmd }]}>
              {/* Mjeno */}
              <View>
                <Text
                  style={[
                ...
                <InputField
                  placeholder={getLangs("input_placeholder_contentname")}
              ...
              <View style={{ marginTop: style.defaultMmd }}>
                ...
                <TextField
                  placeholder={getLangs("input_placeholder_description")}
                  userLinkable
              ...
              <View style={{ marginTop: style.defaultMmd }}>
                ...
                <View style={{ maxHeight: 58 }}>
                  ...
                  >
                    <View style={[styles.timesIcon, style.allCenter]}>
                      <SVG_Time fill={style.colors.blue} />
                    </View>
                    <View
                      style={[
                        ...
                        <DateTimePicker
                          ...
                          onChange={(ev, selectedDate) =>
                            setEvent((cur) => {
                              return {
                                ...cur,
                                starting: selectedDate.getTime(),
                              };
                            })
                          }
                          minimumDate={new Date(Date.now())}
                          maximumDate={new Date(2100, 0, 0, 0, 0, 0)}
                        />
                      ) : (
                        <>
                          <Pressable
                          ...
                          >
                            <Text style={[style.Tmd, style.tWhite]}>
                              {event.starting
                                ? convertTimestampToDate(event.starting)
                                : getLangs("eventcreate_date")}
                            </Text>
                          </Pressable>
                          <Pressable
                            style={[
                          ...
                          >
                            <Text style={[style.Tmd, style.tWhite]}>
                              {event.starting
                                ? convertTimestampToTime(event.starting)
                                : getLangs("eventcreate_time")}
                            </Text>
                          </Pressable>
                        </>
                      )}
                ...
                <View style={{ marginTop: style.defaultMsm }}>
                  <View style={{ maxHeight: 58 }}>
                    ...
                    >
                      <View style={[styles.timesIcon, style.allCenter]}>
                        <SVG_Flag fill={style.colors.blue} />
                      </View>
                      <View
                        style={[
                          styles.timesInnerContainer,
                      ...
                      >
                        {Platform.OS === "ios" ? (
                          <DateTimePicker
                            ...
                            minimumDate={
                              new Date(
                                event.starting ? event.starting : Date.now()
                              )
                            }
                            maximumDate={new Date(2100, 0, 0, 0, 0, 0)}
                          />
                          ...
                          <>
                            ...
                            >
                              <Text style={[style.Tmd, style.tWhite]}>
                                {event.ending
                                  ? convertTimestampToDate(event.ending)
                                  : getLangs("eventcreate_date")}
                              </Text>
                            </Pressable>
                            <Pressable
                            ...
                            >
                              <Text style={[style.Tmd, style.tWhite]}>
                                {event.ending
                                  ? convertTimestampToTime(event.ending)
                                  : getLangs("eventcreate_time")}
                              </Text>
                            </Pressable>
                          </>
          ...
          <View style={styles.sectionContainer}>
            ...
            <View style={styles.underSectionContainer}>
              ...
              {/* Type */}
              <View
                style={[styles.underSectionContainer, styles.checkContainer]}
              >
                <Check
                  color={1}
                ...
                />
                <Text style={[style.tWhite, style.Tmd, style.pH]}>
                ...
                <Check
                  ...
                  onPress={() => {
                    setCheckedCategories((prev) => {
                      return {
                        ...prev,
                        entrance_fee: !prev.entrance_fee,
                      };
                    });
                  }}
                />
                <Text style={[style.tWhite, style.Tmd, style.pH]}>
                  {getLangs("eventcreate_eventdata_entrancefee")}
                </Text>
              </View>
              {/* Website */}
              <View
          ...
          {checkedCategories.type ? (
            <View style={styles.sectionContainer}>
              ...
              <View style={{ marginTop: style.defaultMsm }}>
                {arraySplitter(Event_Types, 1).map((list, listKey) => (
                  <View key={listKey} style={styles.typeItemListContainer}>
                    {list.map((type, key) => (
                      <SelectableButton
                        key={key}
                        title={getLangs(type)}
                        checked={
                          event.eventOptions.type !== undefined
                            ? event.eventOptions.type ===
                              Event_Types.indexOf(type)
                            : false
                        }
                        style={[styles.typeItem, { flex: 1 }]}
                        onPress={() => {
                          setEvent((prev) => {
                            return {
                              ...prev,
          ...
          {checkedCategories.website ? (
            <View style={styles.sectionContainer}>
              ...
              <View style={[style.pH, { marginTop: style.defaultMmd }]}>
                ...
                <InputField
                  placeholder={getLangs("eventcreate_website_inputplaceholder")}
                  keyboardType="url"
                  maxLength={128}
                ...
                >
                  {event.eventOptions.adBanner !== undefined ? (
                    ...
                    >
                      ...
                      />
                      <Text style={[style.Tmd, style.tBlue, styles.hintText]}>
                        {getLangs("eventcreate_adbanner_imghint")}
                      </Text>
                    </View>
                  )}
                </View>
            ...
            <View style={styles.sectionContainer}>
              ...
              {Platform.OS === "ios" ? (
                <View style={{ marginTop: style.defaultMmd }}>
                  ...
                  >
                    {splitArrayIntoNEqualy(Event_Tags, tagLineAmt).map(
                      (line, lineKey) => (
                        <View key={lineKey} style={styles.tagLineContainer}>
                          {line.map((tag, key) => (
                            <Tag
                              key={key}
                              checked={
                                event.eventOptions.tags !== undefined
                              ...
                              onPress={() => {
                                ...
                                if (!t.includes(Event_Tags.indexOf(tag)))
                                  ...
                                  t.splice(
                                    t.indexOf(Event_Tags.indexOf(tag)),
                                    1
                                  );
                ...
                <View style={styles.androidTagsView}>
                  {Event_Tags.map((tag, key) => (
                    <Tag
                      ...
                      onPress={() => {
                        let t = [].concat(
                          event.eventOptions.tags ? event.eventOptions.tags : []
                        );
                        if (!t.includes(Event_Tags.indexOf(tag)))
                          t.push(Event_Tags.indexOf(tag));
          ...
          <View style={[style.allCenter, styles.button]}>
            <EnterButton onPress={publishEvent} checked={buttonChecked} />
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
