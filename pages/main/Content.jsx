...
const client = new MeiliSearch({
  host: "http://192.168.178.104:7700",
});
          ...
          {randomContent.length !== 0 && RANDOM_CONTENT_ENABLED ? (
            <View style={styles.sectionContainer}>
              ...
              <View style={{ marginTop: style.defaultMsm }}>
                {splitterForContent(randomContent, 3).map((line, lineKey) => (
                  <View key={lineKey} style={styles.randomContentLineContainer}>
                    {line.map((item, key) =>
                      item.type === 0 ? (
                        <Post
                          key={key}
                          id={item.id}
                          style={styles.randomContentElement}
                          onPress={() =>
